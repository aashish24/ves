...
vtkSmartPointer<vtkMatrix4x4> toMatrix(const Eigen::Vector4f &origin,
...
//----------------------------------------------------------------------------
vtkSmartPointer<vtkPolyData> vtkPCLConversions::PolyDataFromPCDFile(const std::string& filename, vtkTransform* transform)
{
  int version;
  int type;
  unsigned int idx;
  ...
  if (transform)
    {
    transform->SetMatrix(toMatrix(origin, orientation));
    }
