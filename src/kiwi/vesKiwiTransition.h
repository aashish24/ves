...
/**\brief A base class for transition animations.
  ...
  * Avoid calling "takeInitial" methods if you can since
  * other transitions queued before this one might change
  * the object's state before this transition begins.
  *
...
public:
  ...
  /// An enumeration of transition queues maintained.
  ...
  /// Called by the application as the transition starts.
  virtual int begin(double t, vesKiwiBaseApp* app);
  /// Called by the application before each rendering pass through the scene graph.
