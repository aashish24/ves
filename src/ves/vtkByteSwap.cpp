...
template <class T> inline void vtkByteSwapRange(T* first, int num)
{
...
inline bool vtkByteSwapRangeWrite(const char* first, int num,
                                  FILE* f, int)
{
...
inline bool vtkByteSwapRangeWrite(const signed char* first, int num,
                                  FILE* f, int)
{
  // No need to swap segments of 1 byte.
...
inline bool vtkByteSwapRangeWrite(const unsigned char* first, int num,
                                  FILE* f, int)
{
  // No need to swap segments of 1 byte.
...
template <class T>
inline bool vtkByteSwapRangeWrite(const T* first, int num, FILE* f, long)
{
...
inline void vtkByteSwapRangeWrite(const char* first, int num,
                                  ostream* os, int)
{
  // No need to swap segments of 1 byte.
  os->write(first, num*static_cast<int>(sizeof(char)));
}
inline void vtkByteSwapRangeWrite(const signed char* first, int num,
                                  ostream* os, int)
{
  // No need to swap segments of 1 byte.
...
inline void vtkByteSwapRangeWrite(const unsigned char* first, int num,
                                  ostream* os, int)
{
  // No need to swap segments of 1 byte.
  os->write(reinterpret_cast<const char*>(first),
...
template <class T>
inline void vtkByteSwapRangeWrite(const T* first, int num,
                                  ostream* os, long)
{
  // Swap and write one value at a time.  We do not need to do this in
  // blocks because the file stream is already buffered.
...
template <class T> inline void vtkByteSwapBERange(T*, int) {}
template <class T>
inline bool vtkByteSwapBERangeWrite(const T* p, int num, FILE* f)
{
  size_t status=fwrite(p, sizeof(T), static_cast<size_t>(num), f);
  return status==static_cast<size_t>(num);
}
template <class T>
inline void vtkByteSwapBERangeWrite(const T* p, int num, ostream* os)
{
...
template <class T> inline void vtkByteSwapLERange(T* p, int num)
{
  vtkByteSwapRange(p, num);
}
template <class T>
inline bool vtkByteSwapLERangeWrite(const T* p, int num, FILE* f)
{
  return vtkByteSwapRangeWrite(p, num, f, 1);
}
template <class T>
inline void vtkByteSwapLERangeWrite(const T* p, int num, ostream* os)
{
...
template <class T> inline void vtkByteSwapBERange(T* p, int num)
{
  vtkByteSwapRange(p, num);
}
template <class T>
...
template <class T> inline void vtkByteSwapLERange(T*, int) {}
template <class T>
inline bool vtkByteSwapLERangeWrite(const T* p, int num, FILE* f)
{
  size_t status=fwrite(p, sizeof(T), static_cast<size_t>(num), f);
...
#define VTK_BYTE_SWAP_IMPL(T)                                                   \
  ...
  void vtkByteSwap::SwapLERange(T* p, int num)                            \
    { vtkByteSwapLERange(p, num); }                                             \
  void vtkByteSwap::SwapBERange(T* p, int num)                            \
    { vtkByteSwapBERange(p, num); }                                             \
  bool vtkByteSwap::SwapLERangeWrite(const T* p, int num, FILE* file)     \
    { return vtkByteSwapLERangeWrite(p, num, file); }                                  \
  bool vtkByteSwap::SwapBERangeWrite(const T* p, int num, FILE* file)     \
    { return vtkByteSwapBERangeWrite(p, num, file); }                                  \
  void vtkByteSwap::SwapLERangeWrite(const T* p, int num, ostream* os)    \
    { vtkByteSwapLERangeWrite(p, num, os); }                                    \
  void vtkByteSwap::SwapBERangeWrite(const T* p, int num, ostream* os)    \
