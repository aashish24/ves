...
{
public:
  vesRenderLeaf(int depth, int bin, vesMatrix4x4f &modelViewMatrix,
                vesMatrix4x4f &projectionMatrix,  vesMaterial &material,
                vesMapper &mapper)
  {
    this->m_depth            = depth;
    this->m_bin              = bin;
  ...
  void render(vesRenderState &renderState, vesRenderLeaf *previous)
  {
    ...
    if (this->m_material != previous->m_material) {
  ...
  // Not implemented.
  vesRenderLeaf   (const vesRenderLeaf&);
  void operator=  (const vesRenderLeaf&);
};
...
public:
 ...
 ~vesRenderStage();
  ...
  void addRenderLeaf(vesRenderLeaf &renderLeaf);
